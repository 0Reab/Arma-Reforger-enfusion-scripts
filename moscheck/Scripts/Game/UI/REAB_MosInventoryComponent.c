...
class REAB_MosInventoryComponent : ScriptedWidgetComponent
{	
	...
	//------------------------------------------------------------------------------------------------
	protected void CheckAmmo(ChimeraCharacter player)
	{	
		int totalMags = 0;
		...
		{	// abstract this block into a method or no?
			count ++;
			if (!wpn)
				continue;
	...
	protected BaseContainerList GetPrefabComponents(ResourceName itemPrefab)
	{
		...
		BaseResourceObject resource = res.GetResource(); // could be null here debug
	...
	protected int PlayerHasItem(IEntity player, ResourceName itemPrefab, int amountGoal=1)
	{
	    ...
	    if (!inventory) return 0; // fail safe idk
	...
	//------------------------------------------------------------------------------------------------
	protected bool hasFrag(IEntity player, ResourceName itemPrefab)
	{
		if (!player || !itemPrefab) return false;
		...
		if (prefabName == itemPrefab)
			return true; // return true when nade slot is empty ???????????????????????????????
